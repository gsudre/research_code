...
clf = RandomForestClassifier(n_estimators=500, max_features=np.sqrt(
    data.shape[1]), max_depth=None, min_samples_split=1)
scores = []
for r in range(0):
    s = cross_validation.cross_val_score(clf, data, labels, cv=loo)
    scores.append(s.mean())
print 'Accuracy in LOOCV: ' + str(np.mean(scores)) + ' +- ' + \
        str(np.std(scores))

clf = clf.fit(data, labels)
tmp = clf.predict(data)
print 'Accuracy in training: ' + \
    str(np.sum(tmp == labels) / float(len(labels)))
