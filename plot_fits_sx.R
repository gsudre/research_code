...
G2 = '"persistent"'
G3 = '"NV"'
data = dtL_striatum_1473
#idx <- (gf_1473$GROUP3==G1 | gf_1473$GROUP3==G2 | gf_1473$GROUP3==G3) & gf_1473$MATCH7==1
#d = data.frame(age=gf_1473[idx,]$AGESCAN, y=colSums(data[g,idx]), subject=as.factor(gf_1473[idx,]$PERSON.x),group=gf_1473[idx,]$GROUP3)
idx <- (a$group3==G1 | a$group3==G2 | a$group3==G3) & a$match_outcome==1
d = data.frame(age=a[idx,]$agescan, y=colSums(data[g,idx]), subject=as.factor(a[idx,]$personx),group=a[idx,]$group3)
...
dgs = subset(dg,group==G3)
initial = getStart(dgs$y,dgs$age)
fitG3 <- tryModel(dgs)
points(dgs$age,dgs$y, xlab='Age', ylab='Surface area', col="green")
betas <- fixef(fitG3)
lines(x,betas[1]/(1+exp((betas[2]-x)/betas[3])),col='green', lwd=3)
print(sprintf('Group %s 99pct of asymptote: %.2f', G3, betas[2]-betas[3]*log(1/.99-1)))
print(sprintf('Group %s xmid: %.2f', G3, betas[2]))
legend("bottomright", c(G1,G2,G3), cex=0.8, col=c("blue","red", "green"),lty=1, lwd=2, bty="n")