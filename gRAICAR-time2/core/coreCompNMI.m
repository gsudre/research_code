...
if ptr <= total
    ...
    for i = 1:long
		...
		for j = 1:2
            ...
            % GS: moved this outside the loop to get numVx in both cases
            numVx = size (comp, 2);
            if j == 1
                comp1 = comp';
                ...
                comp2 = comp';
                % sometimes the second time series might be longer
                lgthPar = max(lgthPar, ceil (numVx^(1/3)));
            end
			clear comp;
		end
        ...
        for c1 = 1:split(1)
            ...
            for c2 = 1:split(2) 
                yy = comp2(:, c2);
                % GS: numVx gets overwritten in that function
