...
% GS: Generating 1D time series to later be correlated with original images
comps = 1:size (obj.result.foundComp);
reproLoad = compWeight (obj, comps);
for id = 1:length (comps)
	cp = comps(id);
    cat_AC = [];
    write_AC = 1;
    fprintf ('loading AC %d\n', cp);
    for sb = 1:obj.setup.subNum
        for tr = 1:obj.setup.trial(sb)
            % GS: the order of loops is inefficent, but it's just a text file
            % being opened, so not very costly
    		...
    		if obj.result.foundComp(cp,pos) ~= 0
                ...
                map = reproLoad(id, pos)*map;
                cat_AC = [cat_AC, map];
            else
                write_AC = 0; % for now, let's just write out ACs that all subjects contributed to
            end
            
            % we don't write it if the result if NaNs
            if sum(isnan(map))==length(map)
                write_AC = 0;
            end
		end
    end
    if write_AC==1
        fprintf ('\twriting AC %d\n', cp);
        fn = sprintf ('%s_AC%d.1D', obj.setup.outPrefix, cp);
        dlmwrite(fn,cat_AC,'\n');
    end
end
