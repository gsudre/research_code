...
subj_file = '/Users/sudregp/data/heritability_change/tensors1020.txt'
r_output_file = 'mean_sampling_1020'
...
for tract in tract_names:
    ...
    for var in var_names:
        ...
        if var == 'PD':
            # this is a special case because it's a vector field, so we have 3
            # points per subject
        ...
        if thisSubjects != goal_subjects:
            ...
            start_pos = m_obj.end()
            # split everything, even though we are only going to look up until
            # however many numbers we need (it's a cheap operation)
            ...
            for p in range(num_points):
                ...
                point_val = [float(num)
                             for num in numbers[cnt:cnt+thisSubjects]]
                cnt += thisSubjects
    ...
    fid.close()
r("save(" + ','.join(tract_var_names) + ", file='" + r_output_file +
  ".gzip',compress=TRUE)")
