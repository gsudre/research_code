...
for maskid in fid:
    ...
    for date_dir in date_dirs:
        ...
        # find the line that has the word 'rage', then parse out the name of
        # the folder that contains the data
        fid = open(txtfile[0])
        for line in fid:
            # not all MPRages start with prage, and rage by itself can match
            # storage
            if line.lower().find('rage_') > 0 or \
               line.lower().find('mprage') > 0:
    ...
    if num_mprages > 0:
        if num_mprages == 1:
            ...
            while scan_num < 1:
                try:
                    scan_num = int(input('Which scan to use?: '))
                except ValueError:
                    print("Not a number")
            scan_num -= 1
        mprage = mprage_dirs[scan_num]
        dest = copy_to % int(maskid.rstrip())
        print('Copying', maskid.rstrip(), 'to', dest)
        cmd_str = 'mkdir -p %s; cp %s/* %s/' % (dest, mprage, dest)
        os.system(cmd_str)
        copied_cnt += 1
    else:
