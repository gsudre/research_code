...
print('Running RF filter')
# keep nfolds=5 to use cross-validation as leaderboard
amlfilter <- h2o.automl(x = x, y = target,
                        training_frame = dtrain, seed=myseed,
                        max_runtime_secs = NULL, max_models = NULL,
                        exclude_algos = c("GBM", "GLM", "DeepLearning", "StackedEnsemble"))
# keep only the variables with relative importance > 0
x = h2o.varimp(amlfilter@leader)$variable[h2o.varimp(amlfilter@leader)$relative_importance>0]
