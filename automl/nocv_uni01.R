...
data.valid = df[idx[1:mylim], ]
data.cv = df[idx[(mylim + 1):nrow(df)], ]
print(sprintf('Using %d samples for validation, %d for CV.',
              nrow(data.valid),
              nrow(data.cv)))

nfolds = 5
fold_numbers <- split(1:nrow(data.cv), cut(seq_along(1:nrow(data.cv)),
                      nfolds, labels = FALSE))
best_models = c()
for (fold in 1:nfolds) {
    print(sprintf('Starting CV (%d / %d)', fold, nfolds))
    test_idx = fold_numbers[as.character(fold)][[1]]
    train_idx = setdiff(1:nrow(data.cv), test_idx)
    data.train = data.cv[train_idx, ]
    data.test = data.cv[test_idx, ]
    data.valid = df[idx[1:mylim], ]  # because it can change in each fold
    x = colnames(df)[grepl(pattern = '^v', colnames(df))] 
    ...
    if (grepl(pattern = 'group', target)) {
        ...
        dvalid[, target] = as.factor(dvalid[, target])
    }
    ...
    best_models = c(best_models, aml@leader)
