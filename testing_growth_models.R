...
##### VERTEX-BASED ANALYSIS SMALL COHORT #####
groups = c('NV','persistent','remission')
thresh=.01
brain_data = thalamusL
nvoxels = dim(brain_data)[2]
model = vector()
nvVSper = vector()
nvVSrem = vector()
perVSrem = vector()
for (v in 1:nvoxels) {
    ...
    for (g in groups) {
        idx <- group==g
        d = data.frame(age=age[idx], sx=group[idx], y=brain_data[idx,v], subject=subject[idx])
        dg<-groupedData(y~age|subject, data=d)
        # try a quadratic fit
        m.quad = try(lme(y ~ age + I(age^2), random=~1|subject, data=dg))
        if (length(m.quad)>1 && summary(m.quad)$tTable[3,5] < thresh) {
...
##### WHOLE DATA ANALYSIS SMALL COHORT#####

# check which model works best for each group
cnt = 0
for (g in groups) {
    idx <- group==g
    d = data.frame(age=age[idx], sx=group[idx], y=rowSums(brain_data[idx,]), subject=subject[idx])
    dg<-groupedData(y~age|subject, data=d)
    # try a quadratic fit
    m.quad = try(lme(y ~ age + I(age^2), random=~1|subject, data=dg))
    if (summary(m.quad)$tTable[3,5] < thresh) {
