...
cp /scratch/${USER}/prs/dti_293_imputed_neuro_updated_clin_04172018_clean.csv .
cp /scratch/${USER}/prs/dti_${dti_mode}_voxelwise_08162017.RData .
...
p = read.table(sprintf("/scratch/%s/1080_rd.txt", imuser))
acme_b=p
acme_p=p
tot_b=p
tot_p=p
...
for r in acme tot ade; do
    cat ${r}_pvals.txt | 3dUndump -master /lscratch/${SLURM_JOBID}/mean_fa_skeleton_mask.nii.gz \
        -mask /lscratch/${SLURM_JOBID}/mean_fa_skeleton_mask.nii.gz -datum float \
        -prefix ${r}_pvals.nii.gz -overwrite -;
    cat ${r}_betas.txt | 3dUndump -master /lscratch/${SLURM_JOBID}/mean_fa_skeleton_mask.nii.gz \
        -mask /lscratch/${SLURM_JOBID}/mean_fa_skeleton_mask.nii.gz -datum float \
