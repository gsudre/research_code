...
fname = '~/data/solar_paper_v2/dti_mean_phenotype_cleanedWithinTract3sd_adhd_nodups_extendedAndNuclear_mvmt_pctMissingSE10_FAbt2.5_nuclear.csv'
out_fname = '~/data/solar_paper_v2/linear_dti_mean_phenotype_cleanedWithinTract3sd_adhd_nodups_extendedAndNuclear_mvmt_pctMissingSE10_FAbt2.5_fixed_nuclear.csv'
p_thresh = .05
...
for (s in sxs) { 
    ...
    for (v in phen_vars) {
        ...
        fit = lme(y ~ sx + sex + age + mvmt + I(mvmt^2), random=~1|famid, data=data, na.action=na.omit)
