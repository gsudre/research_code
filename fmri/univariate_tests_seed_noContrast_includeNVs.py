...
if my_test=='inattWithNVs':
    ...
    mne.stats.spatio_temporal_cluster_test([X,Y,Z], n_jobs=2, threshold=thresh, connectivity=connectivity, tail=0, stat_fun=inatt_linreg, n_permutations=nperms, verbose=True)
elif my_test=='hiWithNVs':
    T_obs, clusters, p_values, H0 = \
    mne.stats.spatio_temporal_cluster_test([X,Y,Z], n_jobs=2, threshold=thresh, connectivity=connectivity, tail=0, stat_fun=hi_linreg, n_permutations=nperms, verbose=True)
elif my_test=='nvVSper':
    n = X.shape[0]+Y.shape[0]
    t_thresh = -stats.distributions.t.ppf(thresh, n-1)
    T_obs, clusters, p_values, H0 = \
    mne.stats.spatio_temporal_cluster_test([X,Y], n_jobs=2, threshold=t_thresh, connectivity=connectivity, tail=0, n_permutations=nperms, verbose=True)
elif my_test=='nvVSrem':
    n = X.shape[0]+Z.shape[0]
    t_thresh = -stats.distributions.t.ppf(thresh, n-1)
    T_obs, clusters, p_values, H0 = \
    mne.stats.spatio_temporal_cluster_test([X,Z], n_jobs=2, threshold=t_thresh, connectivity=connectivity, tail=0, n_permutations=nperms, verbose=True)
elif my_test=='perVSrem':
    n = Y.shape[0]+Z.shape[0]
    t_thresh = -stats.distributions.t.ppf(thresh, n-1)
    T_obs, clusters, p_values, H0 = \
    mne.stats.spatio_temporal_cluster_test([Y,Z], n_jobs=2, threshold=t_thresh, connectivity=connectivity, tail=0, n_permutations=nperms, verbose=True)
elif my_test=='nvVSadhd':
    ADHD = np.vstack([Y,Z])
    n = ADHD.shape[0]
    t_thresh = -stats.distributions.t.ppf(thresh, n-1)
    T_obs, clusters, p_values, H0 = \
    mne.stats.spatio_temporal_cluster_test([X,ADHD], n_jobs=2, threshold=t_thresh, connectivity=connectivity, tail=0, n_permutations=nperms, verbose=True)
