...
fname = sprintf('~/data/heritability_change/rsfmri_AROMA%s_%dmin_best2scansSlopes_n%d_%s.RData',
                        pipe, tmin, nrow(res), today)
save(res, file=fname)
...
fname = sprintf('~/data/heritability_change/rsfmri_AROMA%s_%dmin_best2scansSlopesClean_n%d_%s.RData',
                        pipe, tmin, nrow(res_clean), today)
...
fname = sprintf('~/data/heritability_change/rsfmri_AROMA%s_%dmin_best2scansFamsSlopes_n%d_%s.csv',
                        pipe, tmin, nrow(res2), today)
write.csv(res2, file=fname, row.names=F, na='', quote=F)
fname = sprintf('~/data/heritability_change/rsfmri_AROMA%s_%dmin_best2scansFamsSlopesClean_n%d_%s.csv',
                        pipe, tmin, nrow(res2_clean), today)
