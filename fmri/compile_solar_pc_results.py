...
dir_name = home + '/data/solar_familyAndRelativesAndAlone_household/'
...
fid = open(home + '/solar/run_again_' + analysis + '.sh', 'w')
for v in run_again:
    fid.write('cd ~/data/solar_familyAndRelativesAndAlone_household; solar run_ica_voxel %s %d' % (analysis.split('IC')[-1], v) + '\n')
fid.close()
