...
fname = '~/data/solar_paper_v2/fmri_2min_5comps_nuclear.csv'
out_fname = '~/data/solar_paper_v2/categoric_fmri_2min_5comps_nuclear.csv'
p_thresh = .05
...
for (v in phen_vars) {
    y=data[,v]
    eval(parse(text=sprintf('sx=%s', s)))
    fit = lme(scale(y) ~ DX + sex + age, random=~1|famid, data=data, na.action=na.omit)
