...
data_fname = '~/data/baseline_prediction/dti_ad_voxelwise_n223_09212018.RData.gz'
clin_fname = '~/data/baseline_prediction/long_clin_0918.csv'
target = 'nvVSadhd'
myseed = 42
...
dti = read.csv('~/data/baseline_prediction/dti_long_09272018.csv')
...
# CHANGE THIS TO USE FAMID! NEED TO IMPORT FAMID TO CLINICAL FILE!
# SHOULD ALSO CHANGE THIS TO OUTPUT 3DCLUST RESULTS REGARDLESS OF SEED, SO WE
# CAN RUN THE SAME FILE FOR BOTH RANDOMIZED AND NON-RANDOMIZED DATA.
...
library(lme4)
for (v in x) {
    mydata = df[, c(target, 'Sex', 'mvmt', 'age_at_scan', 'numVolumes')]
    mydata$y = df[,v]
    fm = as.formula(sprintf("y ~ %s + Sex + mvmt + I(mvmt^2) + age_at_scan + I(age_at_scan^2) + (1|numVolumes)", target))
    fit = lmer(fm, data=mydata, na.action=na.omit)
...
# then shuffle targets but keep data constant so we can keep the data and
# covariates relationship constant
