...
fname = sprintf('~/data/baseline_prediction/prs_start/varimp_R2_%s_%s_%s_%d_%d.csv',
                reg_model, mymod, my_sx, nfolds, nreps)
write.csv(cbind(a$importance, b$importance), file=fname)

# export variable importance
fname = sprintf('~/data/baseline_prediction/prs_start/fit_R2_%s_%s_%s_%d_%d.Rdata',
                reg_model, mymod, my_sx, nfolds, nreps)
save(fit, file=fname)