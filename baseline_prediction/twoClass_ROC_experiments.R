...
for (test_rows in 1:length(y)) {
    ...
    #                function(x) {wilcox.test(X_train[idx, x],
    #                X_train[!idx, x], exact=F)$p.value})
    pvals = sapply(1:ncol(X_train),
                   function(x) {t.test(X_train[idx, x],
                                           X_train[!idx, x])$p.value})
    good_vars = which(pvals < .1)
    if (length(good_vars) < 2) {
        good_vars = 1:ncol(X_train)
    }
    # good_vars = 1:ncol(X_train)
    print(length(good_vars))
