...
bands = [[4,8]]#[[1, 4], [4, 8], [8, 13], [13, 30], [30, 55], [65, 100]]
...
for b in range(len(bands)):
    X_meg = np.vstack([np.array(g1_meg[b]), np.array(g2_meg[b])])
    X_meg = np.squeeze(X_meg)
    imp = preprocessing.Imputer(missing_values='NaN', strategy='mean', axis=0)
    imp.fit(X_meg)
    X_meg = imp.transform(X_meg)
    X_meg = preprocessing.scale(X_meg)

    # preds is a list of lists, where the first index is the seed, and inside that list there is one prediction per fold. Same thing for certainty
    fmri_preds = []
    fmri_certainty = []
    meg_preds = []
    meg_certainty = []
    ...
    fname = '%s_fmri_meg_%d-%d_%sVS%s'%(seed,bands[b][0],bands[b][1],g1,g2)
