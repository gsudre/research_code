...
if [ ! -e ${mri_path}/${subj_name}/ortho.shape ]; then
    ...
    mv $ds/SAM/good_data $ds/SAM/good_data_nZ
    if [ -e $ds/bad.segments ]; then 
        mv $ds/bad.segments $ds/bad.segments.old
    fi
    SAMcov -r $ds -f "1 58" -m good_data_nZ -v | tee -a $log_file
    ...
    SAMsrc -r $ds -c good_data_nZ,1-58Hz -t targets -u 4 -v -n -Z -W 0 | tee -a $log_file
    echo 'Converting weights'
    dumpSAMFile ${ds}/SAM/good_data_nZ,1-58Hz,targets.wts > tmp
