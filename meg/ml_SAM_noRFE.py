...
bands = [4]*10#range(len(band_names))
...
for b in range(nbands):
    ...
    n_splits=50
    test_size = .2
    cs = [.1, .5, 1, 5, 10, 50, 100]#np.arange(.1,10,.5)
    cv = LeaveOneOut(nobs)
    scores = []
    preds = []
    rand_preds = []
    ...
    for oidx, (train, test) in enumerate(cv):
        ...
        for c in cs:
            for iidx, (itrain, itest) in enumerate(inner_cv):
                ...
                cur_c_val_scores.append(recall_score(y_val, pred))
        ...
        print 'Best C=%.2f, nfeats=%d'%(best_c, clf.transform(X_train).shape[1])
