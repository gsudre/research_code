...
mri_path='/mnt/shaw/MEG_structural/afni/'
...
if [ ! -e ${mri_path}/${subj_name}/ortho.shape ]; then
    ...
    Vecwarp -apar ${mri_path}/${subj_name}/ortho+tlrc -input brainTargetsInTLR888.txt -backward > tmp
    Vecwarp -matvec toPRI.txt -input tmp > ${ds}/SAM/targets8mm
    for b in "${bands[@]}"; do
        split_band=(`echo $b | tr '-' ' '`)
        lb=${split_band[0]}
        hb=${split_band[1]}
        # OK to leave out if we already have computed the covariance before!
        # # this version relaxes the limited bandwith problem
        # /usr/local/neuro/bin/SAMcov -r $ds -f "$lb $hb" -m good_data -v | tee -a $log_file
        SAMsrc -r $ds -c good_data,${lb}-${hb}Hz -t targets8mm -u 4 -v -Z -W 0 | tee -a $log_file
        if [ -e ${ds}/SAM/good_data,${lb}-${hb}Hz,targets8mm.wts ]; then
            echo 'Converting weights'
            dumpSAMFile ${ds}/SAM/good_data,${lb}-${hb}Hz,targets8mm.wts > tmp
