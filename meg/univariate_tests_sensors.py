...
data_dir = '/Users/sudregp/data/meg/connectivity/sensor/'
lmethod = 'pca_flip'
cmethod = 2
alpha=.05
fdr = .05  # 0 for none
...
g3_hi = []
for s in subjs:
    fname = data_dir + '%s-sensor-pli-imcoh-plv-wpli-pli2_unbiased-wpli2_debiased.npy'%(s)
    conn = np.load(fname)[()]
    for b in range(len(bands)):
        data = conn[cmethod,:,:,b]
        # remove the 2 sensors that are not there for everybody
        if data.shape[1]==273:
            keep = [True]*273
            keep[32] = False
            keep[111] = False
