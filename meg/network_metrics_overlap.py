...
for bidx,band in enumerate(bands):
    ...
    X = np.vstack([np.array(g3_data[bidx]), np.array(g2_data[bidx])])
    y = np.hstack([np.zeros([len(g3_data[bidx])]),np.ones([len(g2_data[bidx])])])
    ...
    for oidx, (train, test) in enumerate(cv):
        ...
        for iidx, (itrain, itest) in enumerate(inner_cv):
            ...
            for nu in nus:
                ...
                clfo = LogisticRegression(penalty="l1", dual=False, class_weight='auto')
                clfn = LogisticRegression(penalty="l1", dual=False, class_weight='auto')
                clf.fit(X_inner_train)
                overlap = clf.predict(X_inner_train)==1
                nonoverlap = clf.predict(X_inner_train)==0
                ...
                for i in range(len(y_val)):
                    if clf.predict(X_val[i])==1:
                        cv_preds.append(clfo.predict(X_val[i]))
                    else:
                        cv_preds.append(clfn.predict(X_val[i]))
                params_acc.append(f1_score(y_val, cv_preds, pos_label=1))
        best_nu = nus[np.argmax(np.mean(params_acc,axis=0))]
        print("Optimal nu : %.2f" % best_nu)
        clf = svm.OneClassSVM(nu=best_nu, kernel="linear")
        clf.fit(X_train)
        overlap = clf.predict(X_train)==1
        nonoverlap = clf.predict(X_train)==0
        clfo.fit(X_train[overlap],y_train[overlap])
        clfn.fit(X_train[nonoverlap],y_train[nonoverlap])
        if clf.predict(X_test[i])==1:
            preds.append(clfo.predict(X_test[i]))
        else:
            preds.append(clfn.predict(X_test[i]))
