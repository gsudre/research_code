...
if [ ! -e ${mri_path}/${subj_name}/ortho.shape ]; then
    ...
    SAMcov -r $ds -f "1 58" -m good_data -v | tee -a $log_file
    ...
    SAMsrc -r $ds -c good_data,1-58Hz -t targets -u 4 -v -W 0 | tee -a $log_file
    echo 'Converting weights'
    dumpSAMFile ${ds}/SAM/good_data,1-58Hz,targets.wts > tmp
