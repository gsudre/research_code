...
nv_rows = [i for i in range(len(gf)) if gf[i]['outcomedsm5']=='"NV"']
rem_rows = [i for i in range(len(gf)) if gf[i]['outcomedsm5'] == '"remission"']
per_rows = [i for i in range(len(gf)) if gf[i]['outcomedsm5'] == '"persistent"']
...
for subj in per_subjects:
    ...
    if len(nv_sex_candidates) > 0 or len(rem_sex_candidates) > 0:
        ...
        while True:
            ...
            if len(nv_scan_candidates) > 0 and len(rem_scan_candidates) > 0:
                found = True
...
mlab.rec2csv(match_bool, csv_file[:-4] + '_matched_on18_dsm5.csv')
