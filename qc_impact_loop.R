...
for (roi in brain_vars) {
  print(roi)
  # results will be qbreaks by metric*3
  res = as.data.frame(matrix(nrow = length(qbreaks), ncol = length(qc_metrics)*3),
                      row.names = sapply(qbreaks, function(x) sprintf('q%d', x)))
  colnames(res) = unlist(lapply(qc_metrics, function(x) c(sprintf('%s_n', x),
                                                          sprintf('%s_tstat', x),
                                                          sprintf('%s_pval', x))))
