...
for t in time:
    ...
    for b in brain:
        for h in hemi:
            ...
            data = load_structural('%s/data/structural/%s_%s%s_NV_SA_QCCIVETlt35_QCSUBePASS_MATCHDIFF_on18_dsm5_2to1.csv' % (os.path.expanduser('~'), t, b, h))
...
np.savez('%s/data/results/structural/perms/perm_verts_corr_NVonly_%04d'%(
        os.path.expanduser('~'), np.random.randint(0, 9999)),
