...
for (g in groups) {
    for (t in time) {
        cat(g,t,'\n')
        if (hemi=='R') {
            load(sprintf('~/data/results/simple/es_%s_%s.RData',g,t))
        } else {
            load(sprintf('~/data/results/simple/es_%s_%s_left.RData',g,t))
        }
        for (o in other) {
            eval(parse(text=sprintf('es = esThalamus%s%s%s',hemi,o,hemi)))
            save(es,file=sprintf('~/data/results/simple/es%s_thalamus2%s_%s_%s.RData',
                                 hemi, o, t, g))
        }
    }
}
