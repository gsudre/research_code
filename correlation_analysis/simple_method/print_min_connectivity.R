get_min_connectivity <- function(other, time, hemi, pct, thresh, to_thalamus) {
    if (to_thalamus) {
        res = read.table(sprintf('~/data/results/simple/perm_conn_bin_thresh%0.2f_%s%sthalamus%s_%s.txt',
                             thresh, other, hemi, hemi, time))
    } else {
        res = read.table(sprintf('~/data/results/simple/perm_conn_bin_thresh%0.2f_thalamus%s%s%s_%s.txt',
                                 thresh, hemi, other, hemi, time))
    }
    tmp = sort(res[,1])
    hist(tmp,main=sprintf('%.2f,%s',length(tmp),time))
