...
def compute_nbs(method, X, Y, THRESH, K = 1000, Z=None):
    ...
    Adapted to use multiple tests by Gustavo Sudre
    Also, only uses lower part of connectivity matrix, to agree with MNE
    ...
    # Only consider elements below lower diagonal due to symmetry
    ind_i, ind_j = np.nonzero( np.tril(np.ones( (N,N) ), -1) )
    ...
    if Z is not None:
        emat = np.zeros( (M, nz) )
        for i in range(nz):
            emat[:,i] = Z[ind2ij[:,0], ind2ij[:,1],i].ravel()

    # Perform test at each edge
    stat = np.zeros( M )
    for i in range(M):
        # print ind2ij[i,0], ind2ij[i,1] 
        if method=='ttest':
            ...
            pval = stats.ttest_ind(cmat[i,:], pmat[i,:], equal_var=True)[1]
        elif method=='linreg':
            pval = stats.linregress(cmat[i,:],Y)[3]
        elif method=='mwu':
            pval = 2 * stats.mannwhitneyu(cmat[i,:], pmat[i,:])[1]
        elif method=='anova':
            pval = stats.f_oneway(cmat[i,:], pmat[i,:], emat[i,:])[1]  
        elif method=='kw':
            pval = stats.kruskal(cmat[i,:], pmat[i,:], emat[i,:])[1]
        stat[i] = 1-pval

    # Threshold   
    ind_t = np.where( stat > THRESH )
    ...
    if Z is not None:
        d_stacked = np.hstack( (cmat, pmat, emat) )

    for k in range(K):
        # Randomize
        indperm = np.random.permutation( nx+ny+nz )
        d = d_stacked[:, indperm].copy()

        #################
        
        # Perform stats at each edge
        stat_perm = np.zeros( M )
        for i in range(M):
            if method=='ttest':
                # # assume independent random samples
                # t = ttest2(d[i,:nx], d[i,nx:nx+ny])
                # if tail=='both':
                #     stat_perm[i] = np.abs(t)
                # elif tail == 'left':
                #     stat_perm[i] = -t
                # elif tail == 'right':
                #     stat_perm[i] = t
                # else:
                #     raise('Tail option not recognized')
                pval = stats.ttest_ind(d[i,:nx], d[i,nx:nx+ny], equal_var=True)[1]
            elif method=='linreg':
                pval = stats.linregress(d[i,:],Y)[3]
            elif method=='mwu':
                pval = 2 * stats.mannwhitneyu(d[i,:nx], d[i,nx:nx+ny])[1] 
            elif method=='anova':
                pval = stats.f_oneway(d[i,:nx], d[i,nx:nx+ny], d[i,nx+ny:nx+ny+nz])[1]
            elif method=='kw':
                pval = stats.kruskal(d[i,:nx], d[i,nx:nx+ny], d[i,nx+ny:nx+ny+nz])[1]
            stat_perm[i] = 1-pval 
     
        # Threshold   
        ind_t = np.where( stat_perm > THRESH )
