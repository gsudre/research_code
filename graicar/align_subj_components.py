...
if freq_band is not None:
    fname = res_dir + subj + '_' + freq_band + '_R??.npz'
else:
    fname = res_dir + subj + '_R??.npz'
...
cc_threshold = .7  # centers[np.argmin(bins)]
...
# remove AICs that have lower than 50% of the max reproducibility index
...
if freq_band is not None:
    fname = res_dir + subj + '_' + freq_band + '_aligned_cc%.2f' % cc_threshold
else:
    fname = res_dir + subj + '_aligned_cc%.2f' % cc_threshold
...
# pl.title('corr_thresh = %.2f (forced)' % cc_threshold)
